...
def test_add_season():
    ...
    assert list(add_season(df)["season"]) == [
        "winter",
        "winter",
        "spring",
        "summer",
...
def peaky_profile():
    ...
    # other peaks
    idx = (
        df.loc[add_season(df)["season"].isin(["spring", "summer"])]
        .sample(6800)
        .index
    )
...
def test_seasonal_peaks(peaky_profile, quantile, expected):
    result = (
        seasonal_peaks(peaky_profile, quantile=quantile)
        .set_index("season")
        .to_dict()["value"]
    )
...
def test_pairwise_seasonal_kstest(
    peaky_summer_profiles, peaky_winter_profiles
):
    result = calculate_seasonal_peaks(
        {"summer": peaky_summer_profiles, "winter": peaky_winter_profiles}
    )
