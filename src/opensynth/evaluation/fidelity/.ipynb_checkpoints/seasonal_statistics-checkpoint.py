...
def seasonal_stats(
    ...
    quantile = 1. - quantile if high_low == "high" else quantile
    ...
    filter_condition = (
        with_season
        .select(pl.exclude("datetime", "season"))
        .quantile(quantile)
...
def print_seasonal_stats(df):
    print(df.pivot(on="name", index="season", values="value", aggregate_function="median"))
