...
class RMSNorm(nn.Module):
    ...
    def forward(self, x: Float[Tensor, 'batch sequence dim']) -> Float[Tensor, 'batch sequence dim']:
...
class SelfAttention(nn.Module):
    ...
    def __init__(self, dim: int, dim_head: int, num_head: int=4):
        ...
        self.to_out = nn.Sequential(
            nn.Linear(self.hidden_dim, self.dim_inout),
        ...
        self.rms_norm_q = nn.Sequential(
            ...
            Rearrange('b l (num_head d) -> b num_head l d', num_head = self.num_head)
        )
    ...
    def forward(self, x: Float[Tensor, 'batch sequence dim']) -> Float[Tensor, 'batch sequence dim']:
        qkv = self.to_qkv(x).chunk(3, dim = 2) # 3 * (batch, sequence, hidden_dim)
        q, k, v = map(
        ...
        sim = einsum(
            q, k,
            'b h l dq, b h n dq -> b h l n'
        )
        sim = sim.softmax(dim = -1) # over keys sequence
        out = einsum(sim, v, 'b h l n, b h n dv -> b h dv l') # shape: (batch, num_head, sequence, dim_head)
        out = rearrange(out, 'b h dv l -> b l (h dv)') # shape: (batch, sequence, hidden_dim)
...
class DecoderBlock(nn.Module):
    ...
    def __init__(
        ...
        self.attn = SelfAttention(dim_base, dim_head=dim_base//self.num_attn_head, num_head=self.num_attn_head)
    ...
    def forward(
        ...
        # attention
        x_copy = x.clone()
        x = x + gate_attn * self.attn(self.ln_1(x) * (1. + scale_attn) + shift_attn)

        # mlp
        x = x + gate_mlp * self.mlp(self.ln_2(x) * (1. + scale_mlp) + shift_mlp)
...
class DenoisingTransformer(nn.Module):
    ...
    """
    MAX_SEQUENCE_LENGTH = 8640 # = 24h at 1s resolution. realistically enough. more = slow.
    def __init__(
        self,
        dim_base: int,
    ...
    def forward(
        ...
        scale, shift, encoded_t = _encoded_t.chunk(3, dim=2) # shape: 3 * (batch, 1, dim_base)
        ...
        pos_emb_seq = rearrange(pos_emb_seq, 'seq dim -> 1 seq dim') # shape: (1, sequence, dim_base)
        x = x + pos_emb_seq
        
        x = self.transformer(x, encoded_t) # shape: (batch, sequence, dim_base)
        
        # final adaln modulation
        scale_final, shift_final = self.final_adaLN_modulation(encoded_t).chunk(2, dim=2) # shape: 2 * (batch, 1, dim_base)
        x = self.final_ln(x) * (1 + scale_final) + shift_final
        
        # final linear
        x = torch.concat((x, x_copy), dim=2) # shape: (batch, sequence, dim_base * 2)
