...
def format_data(
    ...
    Args:
        ...
        kwh_col (str): Name of the MW column
    ...
    df.rename(
        columns={datetime_col: "DateTime", kwh_col: "kwh", id_col: "ID", feature_cols[0]: "temperature"},
        inplace=True,
    )
...
def filter_missing_kwh(
    ...
    if len(df_out) == 0:
        raise ValueError(
            "No data left after filtering days with missing MW readings"
        )
    return df_out
...
def get_mean_and_std(df: pd.DataFrame) -> Tuple[float, float, float, float]:
    """
    ...
    return mean, std, temperature_mean, temperature_std
...
def preprocess_pipeline(
    ...
    Args:
        ...
        kwh_col (str, optional): MW column. Defaults to "demand".
        id_col (str, optional): Node ID column. Defaults to "id".
    ...
    mean_std_dict = {"mean": mean, "stdev": stdev, "temperature_mean": temperature_mean, "temperature_std": temperature_std}
